// src/components/courses/AddCoursePage.jsx
...
export default function AddCoursePage({ onNavigate }) {
  ...
  const save = (e) => {
    e.preventDefault();
    if (!course.title.trim() || !course.url.trim() || !course.description.trim()) {
      window.alert("Please fill Title, Description and URL.");
      return;
    }
    const tags = tagsText.split(",").map((t) => t.trim()).filter(Boolean);
    const payload = { ...course, tags };

    const next = [payload, ...(Array.isArray(courses) ? courses : [])];
    setCourses(next);
    try { localStorage.setItem("courses_v1", JSON.stringify(next)); } catch {}
    window.alert(`Course saved!\nShare this Course ID with students:\n${payload.id}`);
    onNavigate("courses");
  };
    ...
    <div className="max-w-3xl mx-auto bg-white p-6 rounded-2xl shadow">
      ...
      <form onSubmit={save} className="space-y-4">
        ...
        <input
          ...
          placeholder="Course URL (Udemy / Coursera / YouTube / Custom)"
          value={course.url}
        ...
        <textarea
          className="w-full px-3 py-2 border rounded-lg"
          placeholder="Teacher Advice (optional)"
          value={course.advice}
          onChange={(e) => setCourse({ ...course, advice: e.target.value })}
        />
      ...
      {/* Optional: show the ID while drafting */}
      <div className="mt-4 text-xs text-gray-500">
        Course ID (share with students): <span className="font-mono">{course.id}</span>
      </div>
    </div>
  );
}
